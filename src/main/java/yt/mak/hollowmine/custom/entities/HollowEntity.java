...
public class HollowEntity extends Animal {
    ...
    public void setTargetPlayer(LivingEntity player) {
    ...
    public void tick() {
        ...
        if (canAttack && targetPlayer != null && !this.level().isClientSide()) {
            double distance = this.distanceTo(targetPlayer);

            if (distance < 3.0) {
                targetPlayer.hurt(level().damageSources().mobAttack(this), 4.0F);
            } else {
